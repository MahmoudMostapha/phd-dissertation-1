...
of moving with respect to this matrix.\footnote{We have already
  ...
  detail in the preceding sections, and since it is reasonable to
  assume that motion associated with growing tumours are relatively
  slow and do not result in significant flow fields, the
  extra-cellular fluid phase has been ignored in this section.} It is
assumed that the solute phases, such as nutrients and enzymes, are
present in sufficient amounts to drive the that allows the cells to
...
\begin{array}{ccc}
\left(\frac{\rho_0^{\mathrm{c}}}{\rho_{0_{\mathrm{ini}}}^{\mathrm{c}}}\right)^{1/2}
& 0 & 0 \\ 0 &
\left(\frac{\rho_0^{\mathrm{c}}}{\rho_{0_{\mathrm{ini}}}^{\mathrm{c}}}\right)^{1/2}
...
(comprised of both cells and matrix) in the reference
configuration. The boundary conditions for the momentum balance
equations prevent rigid body motion of the domain, and do not
constrain the constrain its deformation in anyway. The mass balance
equation is solved with uniform source term
$\Pi^{\mathrm{c}}=0.001$~kg.m$^{-3}$/day. The initial solid
concentration is $\rho_{0_{\mathrm{ini}}}^{\mathrm{c}}=1~$kg.m$^{-3}$.

As a result of this uniform mass source and increasing solid
...
direction of the velocity. Since there are no constraints on the
...
10~mm, and thus giving it 0.5~mm to grow before it contacts the wall.
...
\noindent where $q$ is the horizontal force induced by the wall, $g$
is the gap between the wall and the approaching tumour, and $A$ and
$B$ are parameters associated with the contact model that control the
magnitude of the force, and sharply the force rises as the tumour
approaches the wall. This model ensures that the velocity
...
velocity vectors are much smaller in the constrained regions close to
the wall. Figure~\ref{tumour-constrained-stress-evolution} shows the
...
Upto this point, we have been studying gross effects, treating the
tumour as one homogeneous solid.
