...
stresses along a given direction restricts the in vitro
growth of tumours along those directions \citep{jain1997}. The
foundational examples discussed in
Sections~\ref{tumour-isotropic-swelling}--\ref{cell-transport} focus 
on different aspects of this confined tumour growth problem and a
combination of all these phenomena are manifested in the final
computation presented in Section~\ref{cacophonous-medley}.

The computational model used in this section is primarily comprised of
the solid collagen making up the extra-cellular matrix (ECM) and
motile tumour cells.\footnote{We have already studied the effects of
  the fluid flow on the mechanics in some detail in the preceding
  sections, and since it is reasonable to assume that motion
  associated with growing tumours are relatively slow and do not
  result in significant flow fields, the extra-cellular fluid phase
  has been ignored in this section.} It is assumed that there is a
sufficient amount of solute phases, such as nutrients and enzymes, to
drive the biochemistry that allows the cells produce more ECM, and to
proliferate. The model geometry, as visualised in
Figure~\ref{tumour-isotropic-swelling-0} is a semicircle, of radius
1~cm initially.

\subsection{Kinematic swelling concomitant with growth}
\label{tumour-isotropic-swelling}

Initially, the cells have no mechanics associated with themselves. The
only equation is the mass balance for the ECM, with a source driven
by current cell presence (everywhere), and the momentum balance for
the ECM. Both of these are in the reference configuration. We now
introduce the growth portion of the deformation gradient

\begin{equation}
\bF^{\mathrm{g}} = \left(
\begin{array}{ccc}
\left(\frac{\rho_0^{\mathrm{c}}}{\rho_{0_{\mathrm{ini}}}^{\mathrm{c}}}\right)^{1/2} & 0 & 0 \\
0 & \left(\frac{\rho_0^{\mathrm{c}}}{\rho_{0_{\mathrm{ini}}}^{\mathrm{c}}}\right)^{1/2} & 0 \\
0 & 0 & 1 \end{array}
 \right)
\end{equation}

\noindent since we are working under a plain strain assumption.

The boundary conditions involve preventing horizontal movement of the
vertical edge, and fixing the centre point of that edge. This is just
to prevent RBM.

$\rho_{0_{\mathrm{ini}}}=1$
$R=0.001$ massunits/timeunits

The ECM uses the Mooney-Rivlin Model with 0.37 and 0.11 MPa.

As a result of this uniform cell concentration and production of the
ECM, and the Fg above, the solid tends to swell. The plots show the
radial displacements at two times, and the evolution of the
area. Since there is no stress preventing the growth of the solid, it
just happily swells outwards.
