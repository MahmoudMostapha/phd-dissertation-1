...
viscoelastic model introduced in the preceding section. This model was
loaded at a strain rate of $\dot{\epsilon}=0.01$ Hz for 8.5~s to a
total strain of 0.085, and then subsequently unloaded at the same rate
back to 0 strain in another 8.5~s. The resulting stress-strain curve
is shown in Figure~\ref{medium-hysteresis-visco-0p01-t0p3}. When
...
interaction between the phases, the poroelastic case starts out
stiffer, and the viscoelastic model fails to capture any of the
dynamic effects arising from the fluid flow.
