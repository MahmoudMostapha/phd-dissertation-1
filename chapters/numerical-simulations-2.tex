...
that one of the longitudinal edges of the tissue is held fixed, while
subjecting the other to the suitable displacement condition. The
lateral edges of the solid remain traction free. For the fluid, there
is no flow relative to the solid at the longitudinal edges, i.e.,
...
\footnote{Since the uniform strain rate load-unload displacement
  ...
  in the velocity fields to allow for the dynamic calculations.} it is
...
to note that beyond a certain strain ($\sim$0.06), the solid is stiff
enough to overcome the oscillatory effects arising from the fluid
flow.
...
phases (in the calculations corresponding to Figures~\ref{medium%
  -hysteresis-dynamic-0p01-d1p037} and \ref{medium-hysteresis-dynamic%
  -0p01-d10p37}), but the frictional interaction between the fluid and
solid phases has now increased by an order of magnitude.
