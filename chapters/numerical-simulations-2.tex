...
a relatively small magnitude ($\bD^{\mathrm{fc}} = 1\mathrm{e}^{-5}
\ \bone$~MPa.s.mm$^{-2}$) because we are currently more interested in
...
coefficient tensor ($\bD^{\mathrm{fc}} = \bone$~MPa.s.mm$^{-2}$) 
is significantly greater than in the previous case,
...
The preliminary examples presented in the preceding section
illustrated some basic aspects of the coupled physics exhibited by
non-reacting two-phase mixtures. They allowed us to determine the
evolution of the fluid flow fields and study their effects on the
mechanics. As mentioned toward the end of Section~\ref{eu-energy-dependent%
-source}, one of the mechanisms underlying the time- and
rate-dependent behaviour of soft collagenous tissues  arises from 
frictional effects due to the stress-driven 
fluid flow through the porous collagenous 
network.  In this section, the material parameters and model geometry
of the computational model are tailored to better represent realistic
experimental data, and .... . In certain cases, these results are
compared to a model
(Section~\ref{eu-viscoelastic-solid}) comprising only of a linearly
viscoelastic solid to discern between the results arising from a
poroelastic model with a viscoelastic model. Of course, realistic
tissues may show a combination of these effects.
